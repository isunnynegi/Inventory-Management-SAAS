...
const input = "px-3 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg outline-none focus:border-primary-400 bg-white dark:bg-gray-800 dark:text-gray-100";
...
function PlanCard({ plan, effectivePlan, isSuperAdmin, onAssign }) {
  ...
  return (
    <div className={`relative rounded-2xl border-2 p-6 transition-all ${
      isActive
        ? "border-primary-500 bg-primary-50/40 dark:bg-primary-900/20 dark:border-primary-600"
      ...
      <div className="flex items-start gap-4 mb-4">
        ...
        <div>
          ...
          <div className="flex items-baseline gap-1 mt-0.5">
            ...
            {plan.priceMonthly > 0 && <span className="text-sm text-gray-500 dark:text-gray-400">/month</span>}
          </div>
          {plan.priceYearly > 0 && (
      ...
      {isSuperAdmin ? (
        ...
        <a
          ...
          className={`block w-full py-2 rounded-xl text-sm font-semibold text-center transition-colors ${
            isActive
              ? "bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 pointer-events-none"
              : "bg-primary-600 text-white hover:bg-primary-700"
          }`}
        >
...
function AdminPanel({ orgId }) {
  ...
  const payMut = useMutation({
    ...
    onSuccess: () => { toast.success("Payment recorded"); qc.invalidateQueries({ queryKey: ["sub-admin", orgId] }); setPayForm({ amount: "", method: "upi", reference: "", notes: "" }); },
    onError: e => toast.error(e.response?.data?.message || "Error"),
  });

  const waiverMut = useMutation({
    mutationFn: d => subscriptionApi.adminSetWaiver(orgId, d),
    onSuccess: () => { toast.success("Waiver updated"); qc.invalidateQueries({ queryKey: ["sub-admin", orgId] }); qc.invalidateQueries({ queryKey: ["subscription-my"] }); },
    onError: e => toast.error(e.response?.data?.message || "Error"),
  });
  ...
  return (
    <div className="space-y-5">
      <div className={card}>
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Current Subscription</h3>
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div><span className="text-gray-500 dark:text-gray-400">Plan:</span> <span className="font-medium text-gray-800 dark:text-gray-200">{sub.planSlug}</span></div>
          <div><span className="text-gray-500 dark:text-gray-400">Status:</span> <StatusBadge status={sub.status} /></div>
          <div><span className="text-gray-500 dark:text-gray-400">Effective Plan:</span> <span className="font-semibold text-primary-600 dark:text-primary-400">{sub.effectivePlan}</span></div>
          <div><span className="text-gray-500 dark:text-gray-400">Cycle:</span> <span className="font-medium text-gray-800 dark:text-gray-200">{CYCLE_LABELS[sub.billingCycle] || sub.billingCycle}</span></div>
          {sub.endDate && <div><span className="text-gray-500 dark:text-gray-400">Expires:</span> <span className="font-medium text-gray-800 dark:text-gray-200">{new Date(sub.endDate).toLocaleDateString("en-IN")}</span></div>}
          {sub.overrideReason && <div className="col-span-2"><span className="text-gray-500 dark:text-gray-400">Waiver note:</span> <span className="italic text-gray-600 dark:text-gray-300">{sub.overrideReason}</span></div>}
        </div>
      </div>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/40 rounded-xl p-5">
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3 flex items-center gap-2">
          <Crown size={16} className="text-amber-600 dark:text-amber-400" /> Plan Override (Waiver)
        </h3>
        <div className="flex gap-3 flex-wrap">
          <select className={`flex-1 min-w-[150px] ${input} border-amber-200 dark:border-amber-800/40`}
          ...
          <input className={`flex-1 min-w-[200px] ${input} border-amber-200 dark:border-amber-800/40`}
      ...
      {/* Record payment */}
      <div className={card}>
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3 flex items-center gap-2"><CreditCard size={16} /> Record Payment</h3>
        <div className="grid grid-cols-2 gap-3">
          <input className={input} type="number" placeholder="Amount (₹)" value={payForm.amount} onChange={e => setPayForm(s => ({ ...s, amount: e.target.value }))} />
          <select className={input} value={payForm.method} onChange={e => setPayForm(s => ({ ...s, method: e.target.value }))}>
            <option value="upi">UPI</option>
            <option value="bank">Bank Transfer</option>
            <option value="cash">Cash</option>
            <option value="other">Other</option>
          </select>
          <input className={input} placeholder="UTR / Reference" value={payForm.reference} onChange={e => setPayForm(s => ({ ...s, reference: e.target.value }))} />
          <input className={input} placeholder="Notes (optional)" value={payForm.notes} onChange={e => setPayForm(s => ({ ...s, notes: e.target.value }))} />
        </div>
      ...
      {sub.payments?.length > 0 && (
        <div className={card}>
          <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Payment History</h3>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700">
                <th className="pb-2 text-left">Date</th><th className="pb-2 text-left">Amount</th>
                <th className="pb-2 text-left">Method</th><th className="pb-2 text-left">Reference</th>
              </tr>
            </thead>
            <tbody>
              {sub.payments.map((p, i) => (
                <tr key={i} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                  <td className="py-2 text-gray-600 dark:text-gray-400">{new Date(p.paidAt).toLocaleDateString("en-IN")}</td>
                  <td className="py-2 font-medium text-gray-800 dark:text-gray-200">₹{p.amount?.toLocaleString("en-IN")}</td>
                  <td className="py-2 text-gray-600 dark:text-gray-400">{p.method}</td>
                  <td className="py-2 text-gray-500 dark:text-gray-500 font-mono text-xs">{p.reference || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function SubscriptionPage() {
  ...
  const assignMut = useMutation({
    ...
    onSuccess: () => { toast.success("Plan assigned"); qc.invalidateQueries({ queryKey: ["subscription-my"] }); qc.invalidateQueries({ queryKey: ["sub-admin"] }); },
    onError: e => toast.error(e.response?.data?.message || "Error"),
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage your StockKart plan and billing</p>
      </div>
          ...
          <div className="text-sm">
            <p className="font-semibold text-gray-800 dark:text-gray-200">
              ...
              {subscription.effectivePlanOverride && (
                <span className="ml-2 text-xs bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-medium">
                  Waiver active
                </span>
              )}
            </p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {plans.map(plan => (
            <PlanCard key={plan.slug} plan={plan} effectivePlan={effectivePlan}
              isSuperAdmin={isSuperAdmin} onAssign={(slug) => assignMut.mutate(slug)} />
          ))}
        </div>
      </div>

      {/* Upgrade info for store admin */}
      {!isSuperAdmin && (
        <div className="bg-gray-50 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700 rounded-xl p-5 text-sm text-gray-600 dark:text-gray-400">
          <p className="font-semibold text-gray-800 dark:text-gray-200 mb-1">How to upgrade?</p>
          ...
          <div className="flex gap-4 mt-3">
            <a href="https://wa.me/918979842966" target="_blank" rel="noopener noreferrer"
              className="text-primary-600 dark:text-primary-400 font-medium hover:underline">WhatsApp us →</a>
            <a href="mailto:support@stockkart.in"
              className="text-primary-600 dark:text-primary-400 font-medium hover:underline">Email support →</a>
          </div>
        </div>
      )}
      ...
      {isSuperAdmin && organization?._id && (
        <div>
          <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100 mb-4">Admin Controls</h2>
