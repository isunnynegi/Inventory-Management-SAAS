...
export default function SuppliersPage() {
  ...
  const mutation = useMutation({
    mutationFn: (d) => modal?.data?.id ? supplierApi.update(modal.data.id, d) : supplierApi.create(d),
    onSuccess: () => { qc.invalidateQueries(["suppliers"]); toast.success("Saved!"); setModal(null); reset(); },
    onError: (e) => toast.error(e.response?.data?.message || "Error"),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => supplierApi.delete(id),
    onSuccess: () => { qc.invalidateQueries(["suppliers"]); toast.success("Deleted"); setConfirm(null); },
    onError: (e) => toast.error(e.response?.data?.message || "Error"),
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        ...
        <Table columns={columns} data={rows} loading={isLoading} emptyMsg="No suppliers found" />
        <Pagination page={page} totalPages={totalPages} onChange={setPage} />
      </Card>
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "add" ? "Add Supplier" : "Edit Supplier"}>
